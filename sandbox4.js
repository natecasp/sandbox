...
//       });
  ...
  var od = document.getElementById('outer-diameter').value;
      ...
      var xcord = (Math.abs(e.target.attrs.x - baseshape[0].attrs.x) * (od/500)).toFixed(3);
      var ycord = (Math.abs(e.target.attrs.y - baseshape[0].attrs.y) * (od/500)).toFixed(3);
  ...
  var od = document.getElementById('outer-diameter').value;
 var baseshape = stage.find('.outer');
  var radius = `<div id="`+e.target.attrs.id+`" style="display:flex;"><p class="title">Diameter</p><input class="radius" value="`+(e.target.width()*e.target.attrs.scaleX) * (od/500)+`"></input><p class="title">X</p><input type="number" class="x" value="`+(Math.abs(e.target.attrs.x - baseshape[0].attrs.x) * (od/500)).toFixed(3)+`"></input><p class="title">Y</p><input type="number" class="y" value="`+(Math.abs(e.target.attrs.y - baseshape[0].attrs.y) * (od/500)).toFixed(3)+`"></input></div>`;
...
$(".y").change(function(e){
 ...
 var od = document.getElementById('outer-diameter').value;
      var baseshape = stage.find('.outer');
      var xcord = (Math.abs(element[0].attrs.x - baseshape[0].attrs.x) * (od/500)).toFixed(3);
    var ycord = (Math.abs(element[0].attrs.y - baseshape[0].attrs.y) * (od/500)).toFixed(3);
 oldLines[2].setAttrs({
   text: xcord,
   x: element[0].attrs.x + 15,
   y: element[0].attrs.y - 15
 })
 oldLines[3].setAttrs({
   text: ycord,
   x: element[0].attrs.x - 30,
   y: element[0].attrs.y + 15
 })   
 }
});
...
$('html').keyup(function(e){
  ...
  $("#"+test.slice(-1)[0].attrs.id).remove();
  var oldLines = stage.find('.' + test.slice(-1)[0].attrs.id);
        for (let i = 0; i < oldLines.length; ++i) {
    oldLines[i].destroy();
  }
  if(ifClick.length != 0){
  $("#"+ifClick[0].attrs.id).remove();
  var oldLines = stage.find('.' + ifClick[0].attrs.id);
        for (let i = 0; i < oldLines.length; ++i) {
    oldLines[i].destroy();
  }
  ifClick[0].destroy();
